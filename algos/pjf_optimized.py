...
class PJF():
  ...
  def export_timeline(self):
    output_path = f"{Path(self.path).stem}_PJF.txt"
  ...
  def main(self):
    ...
    pending_processes = sorted(self.processes, key=lambda x: x.gz)

    while any(p.state != "finished" for p in self.processes):

        ready_queue = [p for p in self.processes if p.gz <= self.simdi and p.state != "finished"]

        if not ready_queue:
            next_arrival = min(p.gz for p in self.processes if p.state != "finished")
            self.simdi = next_arrival
            continue
        prio_map = {'high': 1, 'normal': 2, 'low': 3}
        p = min(ready_queue, key=lambda x: (prio_map[x.priority], x.gz))
        ...
        print(f"[ {start_time} ]-- {p.ID} (iz:{p.iz}) -- [ {finish_time} ]")
