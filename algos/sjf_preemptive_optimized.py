...
class SJF_Preemptive():
  ...
  def export_timeline(self):
    output_path = f"{Path(self.path).stem}_SJF_Preemptive.txt"
    ...
    try:
        ...
        print(f"Timeline with IDLE exported to {output_path}")
  ...
  def main(self):
    ...
    while (len ([p for p in self.processes if p.state != 'finished']) != 0):
      ...
      ram = sorted(ram, key=lambda x: x.iz)
      p = ram[0]
